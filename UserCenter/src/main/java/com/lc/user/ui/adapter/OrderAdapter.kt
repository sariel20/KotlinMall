...
class OrderAdapter(context: Context) : BaseRecyclerViewAdapter<Order, OrderAdapter.ViewHolder>(context) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (model.orderGoodsList.size == 1) {//单个商品
            holder.itemView.mSingleGoodsView.setVis(true)
            holder.itemView.mMultiGoodsView.setVis(false)//单个商品隐藏多个商品视图
            ...
            holder.itemView.mSingleGoodsView.setVis(false)//多个商品隐藏单个商品视图
            holder.itemView.mMultiGoodsView.setVis(true)
    ...
    private fun setOptVisible(confirmVisible: Boolean, waitPayVisible: Boolean, cancelVisible: Boolean, holder: ViewHolder) {
        holder.itemView.mConfirmBtn.setVis(confirmVisible)
        holder.itemView.mPayBtn.setVis(waitPayVisible)
        holder.itemView.mCancelBtn.setVis(cancelVisible)

        if (confirmVisible or waitPayVisible or cancelVisible) {
            holder.itemView.mBottomView.setVis(true)
        } else {
            holder.itemView.mBottomView.setVis(false)
        }
